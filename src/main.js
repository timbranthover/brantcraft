...
@vertex fn vs(@builtin(vertex_index)i:u32)->O{var o:O;var p=vec2<f32>(-1.0,-1.0);if(i==1u){p=vec2<f32>(3.0,-1.0);}else if(i==2u){p=vec2<f32>(-1.0,3.0);}o.p=vec4<f32>(p,0.0,1.0);o.uv=p;return o;}
...
fn n3(p:vec3<f32>)->f32{let i=floor(p);let f=fract(p);let w=f*f*(3.0-2.0*f);return mix(mix(mix(h31(i),h31(i+vec3<f32>(1,0,0)),w.x),mix(h31(i+vec3<f32>(0,1,0)),h31(i+vec3<f32>(1,1,0)),w.x),w.y),mix(mix(h31(i+vec3<f32>(0,0,1)),h31(i+vec3<f32>(1,0,1)),w.x),mix(h31(i+vec3<f32>(0,1,1)),h31(i+vec3<f32>(1,1,1)),w.x),w.y),w.z);}
...
fn rotY(p:vec3<f32>,a:f32)->vec3<f32>{let c=cos(a);let s=sin(a);return vec3<f32>(c*p.x-s*p.z,p.y,s*p.x+c*p.z);}
...
@fragment fn fs(i:O)->@location(0)vec4<f32>{
 let frag=(i.p.xy/u.res.xy)*2.0-vec2<f32>(1.0);let asp=u.res.x/u.res.y;let q=vec2<f32>(frag.x*asp,-frag.y);let yaw=u.ang.x;let pitch=u.ang.y;let f=normalize(vec3<f32>(sin(yaw)*cos(pitch),sin(pitch),cos(yaw)*cos(pitch)));let r=normalize(cross(f,vec3<f32>(0.0,1.0,0.0)));let up=cross(r,f);let rd=normalize(f+r*q.x*0.82+up*q.y*0.82);let ro=u.cam.xyz;
 ...
 const mod=device.createShaderModule({code});const info=await mod.getCompilationInfo();const errors=info.messages.filter(m=>m.type==='error');if(errors.length){throw Error('WGSL: '+errors.map(m=>`L${m.lineNum}:${m.linePos} ${m.message}`).join(' | '));}const pipe=await device.createRenderPipelineAsync({layout:'auto',vertex:{module:mod,entryPoint:'vs'},fragment:{module:mod,entryPoint:'fs',targets:[{format}]},primitive:{topology:'triangle-list'}});const bg=device.createBindGroup({layout:pipe.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:U}}]});progress.style.width='72%';
